...
function seed() {
  ...
  const chal1: Challenge = {
    id: 'c-001',
    degreeId: 'd-001',
    title: 'The Buggy Leaderboard',
    type: 'code_fix',
    configPayload: { /* populated via AAI */ },
    status: 'published',
    createdAt: new Date(),
    updatedAt: new Date()
  };

  const chal2: Challenge = {
    id: 'c-002',
    degreeId: 'd-002',
    title: 'Hierarchy Rescue',
    type: 'design_review',
    configPayload: {},
    status: 'published',
    createdAt: new Date(),
  ...
  const chal3: Challenge = {
    id: 'c-003',
    degreeId: 'd-003',
    title: 'The Spaza Shop Ledger',
    type: 'ledger_sort',
    configPayload: {},
    status: 'published',
    createdAt: new Date(),
  ...
  challenges.set(chal1.id, chal1);
  challenges.set(chal2.id, chal2);
  challenges.set(chal3.id, chal3);
  ...
  submissions.set('s-002', {
    ...
    challengeId: 'c-002',
    applicantId: 'a-002',
    payload: { figmaUrl: 'https://figma.com/file/abc', notes: 'Increased title size, grouped info' },
    score: 72,
  ...
  submissions.set('s-003', {
    ...
    challengeId: 'c-003',
    applicantId: 'a-003',
    payload: { buckets: { Assets: ['t3', 't7'], Liabilities: ['t4', 't9'] } },
    score: 90,
...
export function deleteUniversity(id: string): boolean {
  // Also delete associated degrees
...
export function deleteDegree(id: string): boolean {
  return degrees.delete(id);
}
...
export function getDashboardStats() {
  ...
  return {
    ...
    averageScore: allSubs.filter(s => s.score !== null).reduce((acc, s) => acc + (s.score ?? 0), 0) /
      Math.max(1, allSubs.filter(s => s.score !== null).length),
    byDegree: {
